...
class autoACB extends Command
{
     ...
     * @var string
     */
    protected $signature = 'command:autoacb';
    ...
    {
        $flag_auto_acb = getSetting('flag_auto_acb');
        if ($flag_auto_acb == "on") {

            $curl = curl_init();
            curl_setopt_array($curl, array(
                CURLOPT_URL => "https://api.vnitshare.com/acb/transactions",
                ...
                CURLOPT_POSTFIELDS => 'username='.getSetting('bank_user_acb').'&password='.getSetting('bank_pass_acb').'&account_number='.getSetting('bank_account_acb'),
            ));
            ...
            if ($err) {
                ...
                if($ChiTietGiaoDich['status'] && $ChiTietGiaoDich['data']['data']['transactions']){
                    foreach ($ChiTietGiaoDich['data']['data']['transactions'] as $value){
                        if ($value['type'] == 'IN' && $value['account'] == getSetting('bank_account_acb')) {
                            $value['description'] = 'ACB-'.$value['transactionNumber'].'-'.$value['description'];
                            ...
                            if(preg_match('/'.$cuphap.' ([0-9]{0,})/', $str, $matches)){
                                ...
                                $check_exit = HistoryBank::where('memo', $value['description'])->first();
                                if(empty($check_exit)){
                                    ...
                                    if(!empty($updateUser)){
                                        ...
                                        HistoryBank::create([
                                            'user_id' => $user_id,
                                            'trans_id' => 'ACB-'.$value['transactionNumber'],
                                            'coin' => $total_dep,
                                            'memo' => $value['description'],
                                            'type' => 'Nạp tiền từ ACB',
                                        ]);
                                        //cong afff
