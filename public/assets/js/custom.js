...
$(document).ready(function () {
    $(".buy-btn").click(function (t) {
        ...
        Swal.fire({
            title: "&nbsp;&nbsp;" + i,
            text: "Nhập số lượng muốn mua:",
            input: "number",
            inputValue: l,
            inputValidator: function (t) {
                return t < 1 ? "Mua số lượng tối thiếu là 1" : t > r ? "Mua số lượng tối đa là " + r : void 0;
            },
            inputPlaceholder: "Nhập số lượng muốn mua",
            inputAttributes: { maxlength: 5 },
            onOpen: function () {
                Swal.getInput().oninput = function (t) {
                    var n = t.target.value < 0 || t.target.value > r ? "del" : "span";
                    Swal.update({ footer: "Tổng tiền:&nbsp;&nbsp;&nbsp;<" + n + ' class="font-weight-bold">' + addCommas((t.target.value || 0) * a) + "đ</" + n + ">" });
                };
            },
            confirmButtonText: '<i class="fa fa-credit-card"></i> MUA',
            cancelButtonText: "BỎ QUA",
            showCancelButton: !0,
            showLoaderOnConfirm: !0,
            footer: 'Tổng tiền:&nbsp;&nbsp;&nbsp;<span class="font-weight-bold">' + addCommas(l * a) + "đ</span>",
            preConfirm: function (t) {
                ...
                return $.ajax({
                    ...
                    success: function(res) {
                        if(!res.status){
                            Swal.showValidationMessage("Mua thất bại: " + res.msg)
                        }else{
                            c = res.order_id;
                        }
                    },
            ...
            t.value &&
                ...
                }).fire({ icon: "success", html: '<div class="pl-2 text-left">Đã tạo đơn&nbsp;<strong>#' + (c || "") + '</strong><br><em class="text-muted font-size-sm">Ấn để xem chi tiết</em></div>' });
        });
    });
    ...
    $(".btn-copy").click(function(){
        ...
        $temp.remove();
        Dashmix.helpers('notify', {type: 'success', icon: 'fa fa-check mr-1', message: 'Đã copy '+$(this).data("clipboard-text")});
    }) ;
    ...
    $("#btn-submit").click(function (t) {
        ...
        e(!0);
        var n = $("#list-id").val();
        if (!n) return e(!1), Swal.fire({ title: "Vui lòng nhập danh sách Business ID", icon: "error" });
        var r = /([0-9]{15,16})/i,
            i = n
                .trim()
                .split("\n")
        ...
        Swal.fire({
            title: "Bạn chuẩn bị check " + i.length + " link limit?",
            icon: "question",
            showCancelButton: !0,
            ...
            !(function t(n) {
                if (n >= i.length) return e(!1), void Swal.fire({ title: "Hoàn thành", text: "Đã check xong!", icon: "success" });
                var c = r.exec(i[n]),
                    o = c ? c[0] : "";
                if (!o) return t(n + 1);
                var u = "";
    ...
    $("#btn-submit-checkxmdn").click(function (t) {
        var e = function (t) {
            $("#btn-submit-checkxmdn, #list-id").prop("disabled", t);
        };
        e(!0);
        var n = $("#list-id").val();
        if (!n) return e(!1), Swal.fire({ title: "Vui lòng nhập danh sách Business ID", icon: "error" });
        ...
        Swal.fire({
            ...
            if (!t.value) return e(!1);
            $("#check-count-total").text(i.length), $("#check-count-error, #check-count-bmchuaxmdn, #check-count-bmxmdn").text("0"), $("#list-bm-chua-xmdn, #llist-bm-xmdn").val("");
            !(function t(n) {
                if (n >= i.length) return e(!1), void Swal.fire({ title: "Hoàn thành", text: "Đã check xong!", icon: "success" });
                var c = r.exec(i[n]),
                    o = c ? c[0] : "";
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if(res.error){
                            ...
                            if(res.status){
                                    $("#check-count-bmxmdn").text(parseInt($("#check-count-bmxmdn").text()) + 1), $("#list-bm-xmdn").val($("#list-bm-xmdn").val() + i[n]);
                            } else {
                                    $("#check-count-bmchuaxmdn").text(parseInt($("#check-count-bmchuaxmdn").text()) + 1), $("#list-bm-chua-xmdn").val($("#list-bm-chua-xmdn").val() + i[n]);
                            }
                            $("#check-count-live").text(parseInt($("#check-count-live").text()) + 1);
